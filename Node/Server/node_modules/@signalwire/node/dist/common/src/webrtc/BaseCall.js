...
const call_1 = require("../util/constants/call");
...
class BaseCall {
    constructor(session, opts) {
        ...
        this.id = '';
        this.state = call_1.State[call_1.State.New];
        this.prevState = '';
        this.channels = [];
        this.role = call_1.Role.Participant;
        this.extension = null;
        this._state = call_1.State.New;
        this._prevState = call_1.State.New;
        ...
        this.options = Object.assign({}, call_1.DEFAULT_CALL_OPTIONS, { audio, video, iceServers, localElement, remoteElement, micId, micLabel, camId, camLabel, speakerId }, opts);
    ...
    invite() {
        this.direction = constants_1.Direction.Outbound;
        this.peer = new Peer_1.default(constants_1.PeerType.Offer, this.options);
        this._registerPeerEvents();
    }
    answer() {
        this.direction = constants_1.Direction.Inbound;
        this.peer = new Peer_1.default(constants_1.PeerType.Answer, this.options);
        this._registerPeerEvents();
    }
    hangup(params = {}, execute = true) {
        this.cause = params.cause || 'NORMAL_CLEARING';
        this.causeCode = params.causeCode || 16;
        this.setState(call_1.State.Hangup);
        const _close = () => {
            this.peer ? this.peer.instance.close() : null;
            this.setState(call_1.State.Destroy);
        };
        if (execute) {
    ...
    setState(state) {
        ...
        this._state = state;
        this.state = call_1.State[this._state].toLowerCase();
        this.prevState = call_1.State[this._prevState].toLowerCase();
        logger_1.default.info(`Call ${this.id} state change from ${this.prevState} to ${this.state}`);
        this._dispatchNotification({ type: constants_1.NOTIFICATION_TYPE.callUpdate, call: this });
        switch (state) {
            case call_1.State.Purge:
                this.hangup({ cause: 'PURGE', causeCode: '01' }, false);
                break;
            case call_1.State.Active: {
                setTimeout(() => {
    ...
    handleMessage(msg) {
        ...
        switch (method) {
            case constants_1.VertoMethod.Answer: {
                this.gotAnswer = true;
                if (this._state >= call_1.State.Active) {
                    return;
                }
                if (this._state >= call_1.State.Early) {
                    this.setState(call_1.State.Active);
                }
                if (!this.gotEarly) {
            ...
            case constants_1.VertoMethod.Media: {
                if (this._state >= call_1.State.Early) {
                    return;
                }
                this.gotEarly = true;
            ...
            case constants_1.VertoMethod.Display:
            case constants_1.VertoMethod.Attach: {
                const { display_name: displayName, display_number: displayNumber, display_direction } = params;
                this.extension = displayNumber;
                const displayDirection = display_direction === constants_1.Direction.Inbound ? constants_1.Direction.Outbound : constants_1.Direction.Inbound;
                const notification = { type: constants_1.NOTIFICATION_TYPE[method], call: this, displayName, displayNumber, displayDirection };
            ...
            case constants_1.VertoMethod.Info:
            case constants_1.VertoMethod.Event: {
                const notification = Object.assign({}, params, { type: constants_1.NOTIFICATION_TYPE.generic, call: this });
                if (!Handler_1.trigger(constants_1.SwEvent.Notification, notification, this.id)) {
                    Handler_1.trigger(constants_1.SwEvent.Notification, notification, this.session.uuid);
                }
                break;
            }
            case constants_1.VertoMethod.Bye:
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (action) {
                case 'bootObj': {
                    ...
                    this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.Join, conferenceName: laName, participantId: Number(conferenceMemberID), role });
                    if (chatChannel) {
                    ...
                    if (modChannel && role === call_1.Role.Moderator) {
                    ...
                    this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.Bootstrap, participants });
                    break;
                }
                case 'add': {
                    this._dispatchConferenceUpdate(Object.assign({ action: call_1.ConferenceAction.Add, callId, index }, helpers_2.mutateLiveArrayData(data)));
                    break;
                }
                case 'modify':
                    this._dispatchConferenceUpdate(Object.assign({ action: call_1.ConferenceAction.Modify, callId, index }, helpers_2.mutateLiveArrayData(data)));
                    break;
                case 'del':
                    this._dispatchConferenceUpdate(Object.assign({ action: call_1.ConferenceAction.Delete, callId, index }, helpers_2.mutateLiveArrayData(data)));
                    break;
                case 'clear':
                    this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.Clear });
                    break;
                default:
        ...
        return __awaiter(this, void 0, void 0, function* () {
            const tmp = {
                ...
                handler: (params) => {
                    ...
                    this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.ChatMessage, direction, participantNumber, participantName, messageText, messageType, messageId: params.eventSerno });
                }
            };
    ...
    _subscribeConferenceModerator(channel) {
        return __awaiter(this, void 0, void 0, function* () {
                    ...
                    switch (data['conf-command']) {
                        case 'list-videoLayouts':
                            if (data.responseData) {
                                ...
                                this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.LayoutList, layouts: JSON.parse(tmp) });
                            }
                            break;
                        default:
                            this._dispatchConferenceUpdate({ action: call_1.ConferenceAction.ModCmdResponse, command: data['conf-command'], response: data.response });
                    }
                }
            };
            const response = yield this.session.vertoSubscribe(tmp)
            ...
            if (helpers_1.checkSubscribeResponse(response, channel)) {
                this.role = call_1.Role.Moderator;
                this._addChannel(channel);
                Object.defineProperties(this, {
                    listVideoLayouts: {
                        configurable: true,
    ...
    _handleChangeHoldStateSuccess(response) {
        response.holdState === 'active' ? this.setState(call_1.State.Active) : this.setState(call_1.State.Held);
        return true;
    }
    ...
    _onRemoteSdp(remoteSdp) {
        ...
        const sessionDescr = webrtc_1.sdpToJsonHack({ sdp, type: constants_1.PeerType.Answer });
        this.peer.instance.setRemoteDescription(sessionDescr)
            .then(() => {
            if (this.gotEarly) {
                this.setState(call_1.State.Early);
            }
            if (this.gotAnswer) {
                this.setState(call_1.State.Active);
            }
    ...
    _onIceSdp(data) {
        ...
        switch (type) {
            case constants_1.PeerType.Offer:
                this.setState(call_1.State.Requesting);
                msg = new Verto_1.Invite(tmpParams);
                break;
            case constants_1.PeerType.Answer:
                this.setState(call_1.State.Answering);
        ...
        this._execute(msg)
            ...
            type === constants_1.PeerType.Offer ? this.setState(call_1.State.Trying) : this.setState(call_1.State.Active);
        })
            .catch(error => {
    ...
    _onMediaError(error) {
        this._dispatchNotification({ type: constants_1.NOTIFICATION_TYPE.userMediaError, error });
        this.hangup({}, false);
    }
    _dispatchConferenceUpdate(params) {
        this._dispatchNotification(Object.assign({ type: constants_1.NOTIFICATION_TYPE.conferenceUpdate, call: this }, params));
    }
    ...
    _init() {
        ...
        this.setState(call_1.State.New);
