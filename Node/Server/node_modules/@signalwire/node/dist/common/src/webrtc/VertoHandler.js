...
class VertoHandler {
    ...
    handleMessage(msg) {
        ...
        const attach = method === constants_1.VertoMethod.Attach;
        ...
        switch (method) {
            case constants_1.VertoMethod.Punt:
                session.disconnect();
                break;
            case constants_1.VertoMethod.Invite: {
                const call = _buildCall();
                call.setState(call_1.State.Ringing);
                this._ack(id, method);
                break;
            }
            case constants_1.VertoMethod.Attach: {
                ...
                if (this.session.autoRecoverCalls) {
                    ...
                    call.setState(call_1.State.Recovering);
                }
            ...
            case constants_1.VertoMethod.Info:
                params.type = constants_1.NOTIFICATION_TYPE.generic;
                Handler_1.trigger(constants_1.SwEvent.Notification, params, session.uuid);
                break;
            case constants_1.VertoMethod.ClientReady:
                params.type = constants_1.NOTIFICATION_TYPE.vertoClientReady;
                Handler_1.trigger(constants_1.SwEvent.Notification, params, session.uuid);
                break;
            default:
            ...
            switch (action) {
                ...
                case 'conference-liveArray-part': {
                    ...
                    if (laChannel && session._existsSubscription(protocol, laChannel)) {
                        ...
                        if (callId !== null) {
                            const notification = { type: constants_1.NOTIFICATION_TYPE.conferenceUpdate, action: call_1.ConferenceAction.Leave, conferenceName: laName, participantId: Number(conferenceMemberID), role };
    ...
    _handleSessionEvent(eventData) {
        switch (eventData.contentType) {
            ...
            case 'layer-info':
                ...
                break;
            case 'logo-info': {
                const notification = { type: constants_1.NOTIFICATION_TYPE.conferenceUpdate, action: call_1.ConferenceAction.LogoInfo, logo: eventData.logoURL };
