...
class Connection {
    ...
    connect() {
        ...
        this._wsClient.onopen = (event) => {
            this._connected = true;
            this._ping();
            return Handler_1.trigger(constants_1.SwEvent.SocketOpen, event, this.session.uuid);
        };
        this._wsClient.onclose = (event) => {
            this._connected = false;
            return Handler_1.trigger(constants_1.SwEvent.SocketClose, event, this.session.uuid);
        };
    ...
    _setTimer(id) {
        this._timers[id] = setTimeout(() => {
            Handler_1.trigger(id, { error: { code: '408', message: 'Request Timeout' } });
            this._unsetTimer(id);
        }, REQUEST_TIMEOUT);
    }
