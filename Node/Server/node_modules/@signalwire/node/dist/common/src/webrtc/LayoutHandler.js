...
const MCULayoutEventHandler = (session, eventData) => {
    ...
    switch (contentType) {
        case 'layer-info': {
            const notification = Object.assign({ action: call_1.ConferenceAction.LayerInfo }, data);
            Handler_1.trigger(constants_1.SwEvent.Notification, notification, session.uuid);
            break;
        }
        case 'layout-info': {
            const notification = Object.assign({ action: call_1.ConferenceAction.LayoutInfo }, data);
            Handler_1.trigger(constants_1.SwEvent.Notification, notification, session.uuid);
            break;
        }
    }
